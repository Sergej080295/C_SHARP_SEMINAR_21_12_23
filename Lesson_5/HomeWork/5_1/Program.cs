...
{ 
    ...
    public static int FindElementByPosition(int[,] array, int x, int y)
    {
        ...
        {
            return -1; // Позиция по рядам выходит за пределы массива
        }
        if (x < 1 || y < 1)
        {
            return -2; // Позиция по колонкам выходит за пределы массива
        }
    ...
    public static bool ValidatePosition(int[,] array, int x, int y)
    {
        if (x > array.GetLength(0) || y > array.GetLength(1))
        {
            return false; // Позиция по рядам выходит за пределы массива
        }
        if (x < 1 || y < 1)
        {
            return false; // Позиция по колонкам выходит за пределы массива
        }
        return true;
    }
    ...
    {
        if (FindElementByPosition(numbers, x, y) == -1)
        {
            Console.WriteLine("Позиция по рядам выходит за пределы массива");
        }
        else if (FindElementByPosition(numbers, x, y) == -2)
        {
